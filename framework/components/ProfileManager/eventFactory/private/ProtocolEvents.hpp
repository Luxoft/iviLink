 ...
 * @date                10.01.2013
 *
 * Contains events generated PMP interaction protocol
 */
...
public:
    CoreProtocolGetAvailableProfileComplementsResponse(PmpCore * core,
            const std::list<std::pair<Profile::Uid, UInt32> > & complements);

private:
    virtual void handleEvent();

    PmpCore * mCore;
    std::list<std::pair<Profile::Uid, UInt32> > mComplements;
    Logger mLogger;
};
