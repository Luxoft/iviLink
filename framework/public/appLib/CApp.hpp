...
namespace iviLink
{
   ...
   public:
      /**
       * Constructor. No information on supported services is passed to iviLink. In order to enable services or disable any of them in run-time setEnabled() method should be used
       */
      CApp();
      
      
      /**
       * Constructor. Information on one supported service is passed to iviLink. In order to enable addtitional services or disable any of them in run-time setEnabled() method should be used
      ...
      /**
       * Constructor. Information on multiple supported services is passed to iviLink. In order to enable addtitional services or disable any of them in run-time setEnabled() method should be used
       ...
       * @param services list of services UIDs
       ...
       * @retval ERROR_APP_NOT_INITED_IN_IVILINK an error occurred during application's initialization
       * @retval LAUNCHED_BY_USER application was launched by user
       * @retval LAUNCHED_BY_IVILINK application was launched by Application Manager after a request from the other side
       */
      ...
      /**
       * Load a service.
       *This method should be called to load a service in case the Application is started by user (see the initDone() callback.
       *loadService method causes execution of the Application and loading of the appropriate service on the other side
       * After a service loaded successfully it is possible to obtain a profile proxy of a profile associated with the service and use profile functionality from the application.
       ...
       * @param launcher is the output parameter providing information on how the application was launched:
       *    app may be launched either by user (in this case services should be loaded with loadService() method call 
