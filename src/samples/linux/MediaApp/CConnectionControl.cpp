...
#ifndef ANDROID
CConnectionControl::CConnectionControl(IGuiController * controller, IMediaSource * source, IPlayer * player, IServer * server) :
    CApp(iviLink::Service::Uid("MediaService")),
#else
CConnectionControl::CConnectionControl(iviLink::Android::AppInfo info, IGuiController * controller, IMediaSource * source, IPlayer * player, IServer * server) :
    CApp(iviLink::Service::Uid("MediaService"), info),
    mAppInfo(info),
#endif //ANDROID
    mCBregistered(false),
...
#ifndef ANDROID
   mPlayerLogic = new CPlayerLogic(controller, source, player, server);
#else
   mPlayerLogic = new CPlayerLogic(mAppInfo, controller, source, player, server);
#endif //ANDROID
}
...
CPlayerLogic* CConnectionControl::getPlayerLogic()
{
    return mPlayerLogic;
}
...
void CConnectionControl::onInitDone(iviLink::ELaunchInfo launcher)
{
    ...
    if (iviLink::LAUNCHED_BY_USER == launcher)
    {
        ...
        registerMediaCallbacks();
        mGuiController->lockGUI();
        if(loadService(iviLink::Service::Uid("MediaService")))
        {
        ...
        {
            LOG4CPLUS_ERROR(msLogger, "MediaService NOT started");
            killProcess(1);
        }
    }
    else
