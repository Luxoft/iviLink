...
{   
   ...
   CBuffer buffer(outgoingData, data.size() + sizeof(UInt32) );
   if (!buffer.write(data) )
   {
      LOG4CPLUS_WARN(msLogger, "Error writing data to buffer");
   }
   CError err = iviLink::Channel::sendBuffer(mChannelID, buffer);
   if (!err.isNoError())
...
{
   //Allocating channel
   mChannelID = allocateChannel("CScreenSharing", this, eRealTime);
   if(mChannelID != 0)
   {
 ...
 */
void CScreenSharingProfileImpl::onBufferReceived(const tChannelId channel, CBuffer const& buffer)
{
