...
namespace {
    Logger msLogger = Logger::getInstance(LOG4CPLUS_TEXT("samples.Profiles.CSeatReceiverProfile"));
    const std::string ch_tag = "Seat channel";
}

CSeatReceiverProfile::CSeatReceiverProfile(iviLink::Profile::IProfileCallbackProxy* pCbProxy)
   : mChannelID(0)
...
CSeatReceiverProfile::~CSeatReceiverProfile()
{
   LOG4CPLUS_TRACE_METHOD(msLogger, __PRETTY_FUNCTION__ );
   if( mChannelID )
...
void CSeatReceiverProfile::onEnable()
{
   LOG4CPLUS_TRACE_METHOD(msLogger, __PRETTY_FUNCTION__ );
...
void CSeatReceiverProfile::onDisable()
{
...
void CSeatReceiverProfile::bufferReceived(const iviLink::Channel::tChannelId channel, iviLink::CBuffer const& buffer)
{
   LOG4CPLUS_TRACE_METHOD(msLogger, __PRETTY_FUNCTION__ );
...
void CSeatReceiverProfile::channelDeletedCallback(const UInt32 channel_id)
{
   LOG4CPLUS_TRACE_METHOD(msLogger, __PRETTY_FUNCTION__ );
...
void CSeatReceiverProfile::connectionLostCallback()
{
