...
namespace {
  Logger log = Logger::getInstance( LOG4CPLUS_TEXT("samples.Profiles.CClimateReceiverProfile"));
}
              ...
              iviLink::Profile::IProfileCallbackProxy* pCbProxy)
{
  ...
  LOG4CPLUS_INFO(log, "ClimateReceiver: createProfile(\"" + id.value() + "\", \"" + sid.value() + "\")");

  return new iviLink::Profile::CProfile(id, sid, new CClimateReceiverProfile(pCbProxy));
}
