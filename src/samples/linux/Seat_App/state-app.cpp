...
namespace {
    ...
    Logger log4 = Logger::getInstance("SeatClientSample.state-app");
...
bool state_app::setup_profiles()
{
    ...
    if( service_manager(this) )
    {
        sender = polymorphic_downcast<ISeatSenderProfile*>( service_manager(this)->getProfile(service_uid, sender_uid) );
        if( !sender )
        {
            LOG4CPLUS_ERROR( log4, "state_app: failed to get ISeatSenderPofile pointer" );
            return false;
        }
        receiver = polymorphic_downcast<ISeatReceiverProfile*>( service_manager(this)->getProfile(service_uid, receiver_uid) );
        if( receiver )
        {
        ...
        {
            LOG4CPLUS_ERROR( log4, "state_app: failed to get ISeatReceiverPofile pointer" );
