...
class StateApp : public iviLink::CApp
{
 public:
 ...
 private:
    ...
    void profileFail(bool sender_, const CError& e);
    bool setupProfiles();
...
void StateApp<S,R>::onInitDone(iviLink::ELaunchInfo launcher)
{
    ...
    if (master)
    {
        if (loadService(serviceUid) && setupProfiles())
        {
            unlockGUI();
            switch2MASTER();
        }
...
template<typename S, typename R>
void StateApp<S,R>::profileFail(bool sender_, const CError& e)
{
    LOG4CPLUS_TRACE_METHOD(log4StateApp(), __PRETTY_FUNCTION__)
...
template<typename S, typename R>
void StateApp<S,R>::initStateApp()
{
   LOG4CPLUS_TRACE_METHOD(log4StateApp(), __PRETTY_FUNCTION__);
...
template<typename S, typename R>
void StateApp<S,R>::unlockGUI()
{
    LOG4CPLUS_TRACE_METHOD(log4StateApp(), __PRETTY_FUNCTION__);
    #ifndef ANDROID
    #else
    system("am broadcast -a com.luxoft.ivilink.samples.unlock");
    #endif //ANDROID
}
