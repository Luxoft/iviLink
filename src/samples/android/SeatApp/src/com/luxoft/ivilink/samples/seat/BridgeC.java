...
class BridgeC {
	...
	void startApp(){
		if(!isNativeStarted){
			(mThread = new Thread(new Runnable(){
				public void run(){
					startApp(ForApp.getLaunchInfo(activity), ForApp.getServicePath(),
							ForApp.getInternalPath(activity), ForApp.getInternalPath(activity)+"/state.pb");
				}
			})).start();
			isNativeStarted = true;
		} else {
			Log.v(tag, "startApp(): native app is already started!");
		}
	}

	private native void startApp(String launchInfo, String serviceRepo, String internalPath, String statePath); 
	...
	public void initRequest(){
		...
		if(!mThread.isAlive()){
			Log.v(tag, "loadState(): native app is fully initialized! Will do initRequest");
			loadState();
		} else {
			poster.postDelayed(new Runnable(){
				public void run(){
					if(!mThread.isAlive()){
						...
						Log.v(tag, "loadState(): native app is not fully initialized yet!");
