...
CScreenSharingViewer::CScreenSharingViewer(iviLink::Android::AppInfo appInfo
   ...
   : CApp(iviLink::Service::Uid("ScreenSharingViewerService"), appInfo)
   , mpJM(pJm)
...
void CScreenSharingViewer::onInitDone(iviLink::ELaunchInfo launcher)
{
   ...
   if (iviLink::LAUNCHED_BY_USER == launcher)
   {
      ...
      if (loadService(iviLink::Service::Uid("ScreenSharingViewerService")))
      {
...
void CScreenSharingViewer::handleError(CError const & error)
{
   LOG4CPLUS_TRACE_METHOD(msLogger, __PRETTY_FUNCTION__);
