...
CScreenSharingServer::CScreenSharingServer(iviLink::Android::AppInfo appInfo
   ...
   : CApp(iviLink::Service::Uid("ScreenSharingServerService"), appInfo)
   , mpJM(pJm)
...
void CScreenSharingServer::onInitDone(iviLink::ELaunchInfo launcher)
{
   ...
   if (iviLink::LAUNCHED_BY_USER == launcher)
   {
      ...
      if (loadService(iviLink::Service::Uid("ScreenSharingServerService")))
      {
...
void CScreenSharingServer::handleError(CError const & error)
{
   LOG4CPLUS_TRACE_METHOD(msLogger, __PRETTY_FUNCTION__);
...
void CScreenSharingServer::sendServerAddress()
{
   ...
   if(ERR_OK == ::getConnectionInformation(&ci))
   {
