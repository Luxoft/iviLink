...
public class SeatActivity extends Activity implements OnClickListener {
	...
	SeatPosition position = new SeatPosition(new Limits(0, 200), new Limits(0, 200), new Limits(
			-40, 80));
	...
	void guiInitialization() {
		...
		seatBack.setBackgroundResource(seatAngleDrawables.get(position.getBackAngleDef()));
	}
	...
	class SeatTouchListener implements OnTouchListener {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			case MotionEvent.ACTION_DOWN:
				...
				return true;
			case MotionEvent.ACTION_MOVE:
				double[] v1 = { event.getRawX() - pivotPoint[0], event.getRawY() - pivotPoint[1] }; // new
																									// vector
				double[] v2 = { touchOld[0] - pivotPoint[0], touchOld[1] - pivotPoint[1] };
				if (Arrays.equals(v1, v2)) {
					return true;
				}
				double cosAngle = (v1[0] * v2[0] + v1[1] * v2[1])
						/ (Math.sqrt(Math.abs(v1[0] * v1[0] + v1[1] * v1[1])) * Math.sqrt(Math
								.abs(v2[0] * v2[0] + v2[1] * v2[1])));
				if (v1[0] > v2[0]) {
	...
	public void onConfigurationChanged(Configuration newConfig) {
		...
		if (ChairSelection.isDriverSelected()) {
			ChairSelection.setDriverBtnActive(getResources().getConfiguration().orientation);
			onHeaterDriverState(heaterDriverValue);
		} else if (ChairSelection.isPassengerSelected()) {
			ChairSelection.setPassengerBtnActive(getResources().getConfiguration().orientation);
	...
	private void moveHorizontally() {
		((LinearLayout) findViewById(R.id.seatLeftOffset))
				.setLayoutParams(new LinearLayout.LayoutParams(0,
						LinearLayout.LayoutParams.FILL_PARENT, weightLimit.fullCheck(100 - position
								.getXOffset())));
	...
	private void moveSeatBack() {
		seatBack.setBackgroundResource(seatAngleDrawables.get(position.getBackAngle()));
	}

	void onHeaterDriverState(int heaterVal) {
		if (ChairSelection.isDriverSelected()) {
			heaterB.setBackgroundResource(R.drawable.heater_selector_0 + heaterVal);
