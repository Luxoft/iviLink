...
CError SystemControllerMsgProxy::requestConnectionEstablished(UInt8 gender)
{
    if (!mpIpc)
   ...
   CError err = writer.write(gender);
   if (!err.isNoError())
      return err;
   ...
   err = mpIpc->request(id, mWriteBuffer, reqSize, mReadBuffer, respSize);
