...
UInt32 CProfileImplementation::allocateChannel(std::string tag) 
{
	LOG4CPLUS_TRACE_METHOD(mLogger, __PRETTY_FUNCTION__);
	return iviLink::Channel::allocateChannel(tag, this);
}

UInt32 CProfileImplementation::allocateChannelAsServer(std::string tag) 
{
	LOG4CPLUS_TRACE_METHOD(mLogger, __PRETTY_FUNCTION__);
	return iviLink::Channel::allocateChannelAsServer(tag, this);
}

UInt32 CProfileImplementation::allocateChannelAsClient(std::string tag) 
{
	LOG4CPLUS_TRACE_METHOD(mLogger, __PRETTY_FUNCTION__);
	return iviLink::Channel::allocateChannelAsClient(tag, this);
}

void CProfileImplementation::deallocateChannel(UInt32 channelID) 
{
	LOG4CPLUS_TRACE_METHOD(mLogger, __PRETTY_FUNCTION__);
	iviLink::Channel::deallocateChannel(channelID);
}

void CProfileImplementation::sendData(char* data, int sizeOfData, UInt32 channelID)
{
	LOG4CPLUS_TRACE_METHOD(mLogger, __PRETTY_FUNCTION__);
	iviLink::Channel::sendBuffer(channelID, data, sizeOfData);
}

void CProfileImplementation::onBufferReceived(const tChannelId channel, CBuffer const& buffer)
{
	LOG4CPLUS_TRACE_METHOD(mLogger, __PRETTY_FUNCTION__);
	iviLink::Android::JniThreadHelper jth(mpJM);
	JNIEnv * env = jth.getEnv();
