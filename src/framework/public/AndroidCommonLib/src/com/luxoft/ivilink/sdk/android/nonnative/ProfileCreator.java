...
public class ProfileCreator {
	...
	private static byte[] createProfile(String libPath, String profileIUID,
			String serviceUID, byte[] nativeCallbacksPointer) {
		Log.v(tag, Logging.getCurrentMethodName(libPath, profileIUID,
				serviceUID, nativeCallbacksPointer));
		try {
			ClassLoader loader = new DexClassLoader("file://" + libPath,
					CApp.internalPath, null,
					ProfileCreator.class.getClassLoader());
			...
			while (allEntries.hasMoreElements()) {
				...
				if (name.endsWith("class")) {
					try {
						Class claz = loader.loadClass(name
								.replace(".class", ""));
						Log.i(tag, "Loaded class: " + claz.getName());
						if (AbstractProfile.class.isAssignableFrom(claz)) {
							Log.i(tag, claz.getName() + " is "
									+ AbstractProfile.class.getName());
							if (!Modifier.isAbstract(claz.getModifiers())) {
								...
								Log.e(tag, claz.getName()
										+ " is abstract, skipping!");
							}
						} else {
							Log.i(tag, claz.getName() + "is not "
									+ AbstractProfile.class.getName());
						}
