...
using AXIS::PMAL::PIM::CProfileInstanceMap;

static const char gModuleName[] = "CProfileInstanceMap";
Logger AXIS::PMAL::PIM::CProfileInstanceMap::msLogger = Logger::getInstance(LOG4CPLUS_TEXT("profileManager.Pmal.Pim.ProfileInstanceMap"));
...
CError CProfileInstanceMap::registerProfile(AXIS::Profile::IUid const& profileUid, AXIS::Profile::CProfileInternal* pInstance)
{
   if (pInstance == NULL)
...
CError CProfileInstanceMap::unregisterProfile(AXIS::Profile::CProfileInternal* pInstance)
{
   if (pInstance == NULL)
         return CError(1, gModuleName, CError::ERROR, "pInstance is NULL");
   ...
   {
      AXIS::Profile::IUid uid = it->second;
...
CError CProfileInstanceMap::find(AXIS::Profile::IUid const& profileUid, AXIS::Profile::CProfileInternal*& pInstance)
{
   CError err = CError::NoError(gModuleName);
...
CError CProfileInstanceMap::find(AXIS::Profile::CProfileInternal* const pInstance, AXIS::Profile::IUid& profileUid)
{
   CError err = CError::NoError(gModuleName);
...
CError CProfileInstanceMap::isKnown(AXIS::Profile::IUid const& profileUid)
{
   CError err = CError::NoError(gModuleName);
...
CError CProfileInstanceMap::isKnown(AXIS::Profile::CProfileInternal* pInstance)
{
   CError err = CError::NoError(gModuleName);
