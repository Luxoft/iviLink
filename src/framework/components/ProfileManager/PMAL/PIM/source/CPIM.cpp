...
#include "framework/components/ProfileManager/PMAL/PIM/include/PIUIDGenerator.hpp"
#include "framework/components/ProfileManager/PMAL/PIM/include/profileFactory.hpp"
#include "framework/components/ProfileManager/PMAL/ipc_protocol/include/IPMALIpcToPIM.hpp"
#include "framework/components/ProfileManager/PMAL/core/include/IPMALCoreToPIM.hpp"
...
class CProfileEnabler
{
   ...
   CProfileEnabler();
public:
   static void enable(Profile::CProfileInternal* p)
   {
   ...
   static void disable(Profile::CProfileInternal* p)
   {
      LOG4CPLUS_TRACE_METHOD(logger, __PRETTY_FUNCTION__ + (" " + convertIntegerToString((intptr_t)p)));
      assert(p);
      p->setEnabled(false);
...
class CProfileDestroyer
{
   ...
   CProfileDestroyer();
public:
   static void destroy(Profile::CProfileInternal* p)
   {
      LOG4CPLUS_TRACE_METHOD(logger, __PRETTY_FUNCTION__ + (" " + convertIntegerToString((intptr_t)p)));
      assert(p);
      delete p;
...
struct CIncomingProfileJobData : public IThreadPoolJobData
{
   ...
   AXIS::Profile::ApiUid mPapiUid;
   AXIS::Profile::IUid mPiuid;
   AXIS::Service::Uid mServiceUID;
};

struct CProfileDiedJobData : public IThreadPoolJobData
{
   CProfileDiedJobData(CPIM* pPim, AXIS::Profile::IUid const& piuid) :
      mpPim(pPim),
      mPiuid(piuid)
   {}
...
CPMALError CPIM::loadProfile(AXIS::Profile::Uid const& profileUid,
...
void CPIM::incomingProfile(AXIS::CUid const& profileUid,
         AXIS::Profile::ApiUid const& papiUid,
         AXIS::Profile::IUid const& piuid,
         AXIS::Service::Uid const& serviceUID)
{
...
void CPIM::profileDied(AXIS::Profile::IUid const& piuid)
{
...
CError CPIM::sessionRequest(AXIS::Service::SessionUid session,
...
void CPIM::unloadProfileThroughClient(IProfileManagerCallbacks* clbs, Profile::CProfile* pp, Profile::IUid const& piuid)
{
...
void CPIM::incomingProfileInternal(AXIS::Profile::Uid const& profileUid,
      AXIS::Profile::ApiUid const& papiUid,
      AXIS::Profile::IUid const& piuid,
      AXIS::Service::Uid const& serviceUID)
{
...
void CPIM::profileDiedInternal(AXIS::Profile::IUid const& piuid)
{
   ...
   unloadProfileThroughClient(clb, static_cast<Profile::CProfile*>(pi), piuid);
}

CPMALError CPIM::generatePIUID(AXIS::CUid const& profileUid,
      AXIS::Service::Uid const& serviceUID,
