...
#include "framework/components/ProfileManager/PMP/interaction/include/CPmpProtocol.hpp"
#include "framework/components/ProfileManager/PMP/interaction/include/CPmpCoreClbCtrl.hpp"
   ...
   namespace PMP
   {
      ...
      void CPmpProtocol::connect()
      {
         ...
         while (mBe)
         {
            ...
            UInt32 chid = 0;
            CError err = AXIS::ChannelSupervisor::allocateChannel(this,mTag.c_str(), chid);
            if (err.isNoError())
            {
               mChannelIdCond.lock();
      ...
      void CPmpProtocol::disconnect(bool final)
      {
         ...
         if (chid != 0)
         {
            AXIS::ChannelSupervisor::deallocateChannel(chid, mTag.c_str());
         }
      }
      ...
      void CPmpProtocol::dataReceivedCallback(const unsigned int channelId, const unsigned int readSize, const unsigned int freeSize)
      {
         LOG4CPLUS_TRACE_METHOD(msLogger, __PRETTY_FUNCTION__ );

         unsigned char * data = new unsigned char [readSize];
         unsigned int recSize = 0;
         CError err =  AXIS::ChannelSupervisor::receiveData(mChannelId,mTag.c_str(),data,recSize,readSize);
         if(!err.isNoError())
         {
            LOG4CPLUS_INFO(msLogger, static_cast<std::string>(err));
            delete [] data;
      ...
      PMPFrame * CPmpProtocol::makeRequest(PMPFrame & requestFrame)
      {
         ...
         CError err = AXIS::ChannelSupervisor::sendData(getChannelId(), mTag.c_str(), reinterpret_cast<unsigned char *>(&requestFrame), size);
         if (!err.isNoError())
         {
            return 0;
         }
      ...
      void CPmpProtocol::makeResponse(PMPFrame & response)
      {
         ...
         AXIS::ChannelSupervisor::sendData(getChannelId(), mTag.c_str(), reinterpret_cast<unsigned char *>(&response), size);
      }
