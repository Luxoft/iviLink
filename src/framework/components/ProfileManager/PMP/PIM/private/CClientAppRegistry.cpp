...
CError CClientAppRegistry::registerApp(CUid const& appId, iviLink::Ipc::DirectionID const& dirId)
{
...
CError CClientAppRegistry::unregisterProfile(iviLink::Profile::IUid const& piuid)
{
   LOG4CPLUS_TRACE_METHOD(msLogger, __PRETTY_FUNCTION__ );
...
CError CClientAppRegistry::findAppByDirId(iviLink::Ipc::DirectionID const& dirId, CUid& appId) const
{
   CError err = CError::NoError(gModuleName);
...
CError CClientAppRegistry::findDirIdByApp(CUid const& appId, iviLink::Ipc::DirectionID& dirId) const
{
   CError err = CError::NoError(gModuleName);
...
CError CClientAppRegistry::findAppByPIUID(iviLink::Profile::IUid const& piuid, CUid& appId) const
{
   CError err = CError::NoError(gModuleName);
...
CError CClientAppRegistry::findSidByPIUID(iviLink::Profile::IUid const& piuid, iviLink::Service::Uid & sid) const
{
   LOG4CPLUS_TRACE_METHOD(msLogger, __PRETTY_FUNCTION__ );
   iviLink::CUid appId;
...
bool CClientAppRegistry::hasInstancesOf(iviLink::Profile::Uid const& uid) const
{
   bool res = false;
...
void CClientAppRegistry::findInstancesOf(iviLink::Profile::Uid const& uid, std::vector<CProfileInstanceData> & result) const
{
   IsProfileUidInProfDataPred pred(uid);
...
bool CClientAppRegistry::isAppHaveService(iviLink::CUid const& appId, iviLink::Service::Uid const& sid) const
{
   bool res = false;
   mAppDatasMutex.lock();
   {
...
void CClientAppRegistry::addServiceToApp(iviLink::CUid const& appId, iviLink::Service::Uid const& sid)
{
   mAppDatasMutex.lock();
   {
      tAppDatas::iterator it = mAppDatas.find(appId);
...
void CClientAppRegistry::removeServiceFromApp(iviLink::CUid const& appId, iviLink::Service::Uid const& sid)
{
   mAppDatasMutex.lock();
   {
      tAppDatas::iterator it = mAppDatas.find(appId);
...
CError CClientAppRegistry::findAppBySid(iviLink::Service::Uid const& sid, iviLink::CUid & appId) const
{
   CError err = CError::NoError(gModuleName);
