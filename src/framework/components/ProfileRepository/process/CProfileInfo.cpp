 ...
 * @brief            Source file for CProfileInfo class (part of Profile Repository Process)
 * @author           VPlachkov
 ***************************************************************************/
...
namespace AXIS
{
   namespace ProfileRepository
   {
      ...
      Logger CProfileInfo::msLogger = Logger::getInstance(LOG4CPLUS_TEXT("profileRepository.Process.CProfileInfo"));
      ...
      void CProfileInfo::loadProfileInfo()
      {
         ...
         {
            mFailed = true;
            LOG4CPLUS_WARN(msLogger, "Error while parsing Profile API manifest");
            return;
         }

         pugi::xml_node prof = doc.child("profile");
         mUid = UID(prof.child_value("uid"));
         mName = prof.child_value("name");
         mVersion = atoi(prof.child_value("version"));
         mApiUid = UID(prof.child("api").attribute("uid").value());
