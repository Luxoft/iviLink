...
* (c) copyright     2012
* Company           LUXOFT
* @file             L0InterfaceStub.cpp
* @brief            The L0InterfaceStub class implementation file
* @author           KSkidanov
***************************************************************************/
/********************************************************************
 *
...
#include "framework/components/ConnectivityAgent/src/generic/L0/include/CSourceAgent.hpp"
#include "framework/components/ConnectivityAgent/src/generic/L0/include/CTargetAgent.hpp"
 ...
 *
 ********************************************************************/
#include "framework/components/ConnectivityAgent/src/generic/L0/include/CChannelAllocator.hpp"
#include "utils/misc/include/Logger.hpp"
#include "utils/misc/include/byteOrder.hpp"
#include "utils/threads/include/CTimeoutManager.hpp"

CMutex L0InterfaceStub::mSingletonMutex;
L0InterfaceStub* L0InterfaceStub::mSelf = 0;

static const char* systemControllerSocket = "SysCtr_ConAgnt";

Logger L0InterfaceStub::logger = Logger::getInstance(LOG4CPLUS_TEXT("ConnectivityAgent.L0.L0InterfaceStub"));

L0InterfaceStub::L0InterfaceStub():
   mpAgent(NULL),
...
void L0InterfaceStub::start(bool isServer, const char* sockPath)
{
   mpAgent = new CConnectivityAgent(isServer);
   mpAgent->start();
   mpIpc = new AXIS::Ipc::CIpc(sockPath, *this);
   CError err = mpIpc->beginWaitForConnection();
   assert(err.isNoError());
}

L0InterfaceStub* L0InterfaceStub::getInstance()
{
   mSingletonMutex.lock();
   if(mSelf == NULL)
   {
      mSelf = new L0InterfaceStub();
   }
   mSingletonMutex.unlock();
   return mSelf;
}

void L0InterfaceStub::deleteInstance()
{
   delete this;
   mSelf = NULL;
}

void L0InterfaceStub::OnConnection(DirectionID dirId)
{
   LOG4CPLUS_INFO(logger, "L0InterfaceStub::OnConnection dirId = "
      + convertIntegerToString(dirId));
   mClientDirections.push_back(dirId);
}

void L0InterfaceStub::OnConnectionLost(DirectionID dirId)
{
   LOG4CPLUS_INFO(logger, "L0InterfaceStub::onConnectionLost dirId = " 
      + convertIntegerToString(dirId));
   std::remove(mClientDirections.begin(), mClientDirections.end(), dirId);
}

ERROR_CODE L0InterfaceStub::tryAllocateChannel(TChannelPriority prio, UInt32 channel_id)
{
   LOG4CPLUS_TRACE_METHOD(logger, "L0InterfaceStub::tryAllocateChannel(" + convertIntegerToString(channel_id) +")");
   ...
   if (mRegistry.find(channel_id) == mRegistry.end()) //if channel wasn't opened yet
   {
      ...
      else
      {
         ...
         {
            LOG4CPLUS_INFO(logger, "L0InterfaceStub::tryAllocateChannel() =>Allocation complete ");
            mRequestedMap.erase(channel_id);
            ret = ERR_REASSIGN;
         }
         else  //channel was requested by Client Application not the first time
         {
            LOG4CPLUS_INFO(logger, "L0InterfaceStub::tryAllocateChannel() => ERROR: allocation is in progress! ");
            ret = ERR_IN_PROGRESS;
   ...
   {
      LOG4CPLUS_INFO(logger, "L0InterfaceStub::tryAllocateChannel() => ERROR: channel already exists! ");
      ret = ERR_NUMBER_BUSY;
   }
...
ERROR_CODE L0InterfaceStub::allocateChannel(TChannelPriority prio, UInt32 channel_id)
{
   LOG4CPLUS_INFO(logger, "L0InterfaceStub::allocateChannel(type = "
   ...
   if (mRegistry.find(channel_id) == mRegistry.end())
   {
      ...
      LOG4CPLUS_INFO(logger, "L0InterfaceStub::allocateChannel():  CChannelAllocator ret = "
   ...
   {
      LOG4CPLUS_ERROR(logger, "L0InterfaceStub::allocateChannel() => ERROR: channel already exists! ");
      ret = ERR_NUMBER_BUSY;
   }
   mRegistryMutex.unlock();
...
ERROR_CODE L0InterfaceStub::deallocateChannel(UInt32 channel_id)
{
   LOG4CPLUS_INFO(logger, "L0InterfaceStub::deallocateChannel(chID = " + convertIntegerToString(channel_id) + ")");
   ...
   {
      LOG4CPLUS_ERROR(logger, "L0InterfaceStub::deallocateChannel(chID = "
...
ERROR_CODE L0InterfaceStub::sendData(CDataAccessor & accessor)
{
   LOG4CPLUS_TRACE(logger, "L0InterfaceStub::sendData()");
   ERROR_CODE ret = ERR_NOTFOUND;
   mRegistryMutex.lock();
   ...
   if (iter!=mRegistry.end())
   {
      ...
      LOG4CPLUS_INFO(logger, "L0InterfaceStub::sendData() => Overall data bytes " + convertIntegerToString(size));
      do
      {
         ...
         LOG4CPLUS_INFO(logger, "L0InterfaceStub::sendData() => sending "
   ...
   {
      LOG4CPLUS_ERROR(logger, "L0InterfaceStub::sendData() => channel not found!");
   }
   mRegistryMutex.unlock();
   return ret;
}

ERROR_CODE L0InterfaceStub::receiveData(Buffer & buffer, UInt32 channel_id)
{
   LOG4CPLUS_INFO(logger, "L0InterfaceStub::receiveData() => channel " + convertIntegerToString(channel_id));
   ...
   ERROR_CODE ret = ERR_UNKNOWN;
      ...
      LOG4CPLUS_INFO(logger, "L0InterfaceStub::receiveData() =>  buf size "
      ...
      if(channel_id == SERVICE_CHANNEL_NUMBER)
      {
         ...
         do
         {
            ...
            offset += 2;
            LOG4CPLUS_INFO(logger, "L0InterfaceStub::receiveData() =>  buf_length " + convertIntegerToString(buf_length));
            ...
            {
               LOG4CPLUS_WARN(logger, "L0InterfaceStub::receiveData() => UNKNOWN SERVICE REQUEST = "
                     + convertIntegerToString(operation) + "!!!");
            }

         } while (offset < buffer.getFilledSize());

         LOG4CPLUS_INFO(logger, "L0InterfaceStub::receiveData() => offset "
...
void L0InterfaceStub::sendRequest( CDataAccessor & accessor)
{
   Buffer buf;
   UInt32 offset = 0;
...
void L0InterfaceStub::OnConnected()
{
   LOG4CPLUS_INFO(logger, "L0InterfaceStub::OnConnected(): Connectivity Agents connected, create service channel ....");
   allocateChannel(eSafetyCritical, SERVICE_CHANNEL_NUMBER);
   mSCProtocol.sendConnectionEstablished();   

}

void L0InterfaceStub::OnDisconnected()
{
   LOG4CPLUS_INFO(logger, "L0InterfaceStub::OnDisconnected(): Connectivity Agents disconnected=>sending notfications to all clients");
...
void L0InterfaceStub::fillCallbacks()
{

   mClientsCallbacksMap.insert(std::pair<tOpCode  ,process_func >(E_ALLOCATE_CHANNEL, &L0InterfaceStub::processClientAllocateRequest));
   mClientsCallbacksMap.insert(std::pair<tOpCode  ,process_func >(E_DEALLOCATE_CHANNEL, &L0InterfaceStub::processClientDeallocateRequest));
   mClientsCallbacksMap.insert(std::pair<tOpCode  ,process_func >(E_SEND_DATA, &L0InterfaceStub::processClientSendRequest));

   mServiceCallbacksMap.insert(std::pair<tOpCode  ,process_func >(E_ALLOCATE_CHANNEL, &L0InterfaceStub::processServiceAllocateRequest));
   mServiceCallbacksMap.insert(std::pair<tOpCode  ,process_func >(E_DEALLOCATE_CHANNEL, &L0InterfaceStub::processServiceDeallocateRequest));
   mServiceCallbacksMap.insert(std::pair<tOpCode  ,process_func >(E_ALLOCATE_CHANNEL_RESP, &L0InterfaceStub::processServiceAllocateResponse));
   mServiceCallbacksMap.insert(std::pair<tOpCode  ,process_func >(E_DEALLOCATE_CHANNEL_RESP, &L0InterfaceStub::processServiceDeallocateResponse));

}

void L0InterfaceStub::OnRequest(AXIS::Ipc::MsgID id, 
      UInt8 const* pPayload, UInt32 payloadSize, 
      UInt8* const pResponseBuffer, UInt32& bufferSize,
      DirectionID dirId)
{
   LOG4CPLUS_INFO(logger, "L0InterfaceStub::OnRequest(): received " + convertIntegerToString(payloadSize) + " bytes");
   ...
   {
      LOG4CPLUS_INFO(logger, "L0InterfaceStub::Receive()");
      ...
      if (E_ALLOCATE_CHANNEL == pReq.getOpCode())
      {
         ...
         {
            LOG4CPLUS_INFO(logger, "L0InterfaceStub::Receive() => set client for requested "
      ...
      else if(E_ALLOCATE_CHANNEL_RESP == pReq.getOpCode())
      {
         ...
         if(iter!=mRegistry.end())
         {
            LOG4CPLUS_INFO(logger, "L0InterfaceStub::Receive() => set client for allocated "
                   + convertIntegerToString(channel_id) + " channel !");
            iter->second.mClientDir = dirId;
         }
   ...
   {
      LOG4CPLUS_WARN(logger, "L0InterfaceStub::Receive() => UNKNOWN REQUEST!");
   }

}

ERROR_CODE L0InterfaceStub::processClientAllocateRequest(CDataAccessor & accessor)
{
   ERROR_CODE ret = ERR_UNKNOWN;
   ...
   if(SERVICE_CHANNEL_NUMBER !=channel_id)
   {
      ...
      LOG4CPLUS_INFO(logger, "L0InterfaceStub::processClientAllocateRequest() => Allocate Channel Request: ChID = "
      ...
      {
         LOG4CPLUS_INFO(logger, "L0InterfaceStub::processClientAllocateRequest() => this is newly requested channel!" );
      ...
      else
      {
         if (ret == ERR_REASSIGN) /// or this is channel waiting for client request
         {
            ...
            LOG4CPLUS_INFO(logger, "L0InterfaceStub::processClientAllocateRequest() => this is channel waited for client request!" );
         }
...
ERROR_CODE L0InterfaceStub::processClientDeallocateRequest(CDataAccessor & accessor)
{
   ERROR_CODE ret = ERR_UNKNOWN;
   UInt32 channel_id = accessor.getChannelID();
   if(SERVICE_CHANNEL_NUMBER !=channel_id)
   {
      LOG4CPLUS_INFO(logger, "L0InterfaceStub::processClientDeallocateRequest() => Deallocate Channel Request: ChID = " + convertIntegerToString(channel_id));
      mRegistryMutex.lock();
      tChannelsRegistryMap::iterator iter =  mRegistry.find(channel_id);
      if (iter!=mRegistry.end())
      {
      ...
      {
         LOG4CPLUS_INFO(logger, "L0InterfaceStub::processClientDeallocateRequest() => Deallocate Channel Request: ChID = " + convertIntegerToString(channel_id) + " NOT FOUND!");
         ret = ERR_NOTFOUND;
      }
      mRegistryMutex.unlock();
   }
...
ERROR_CODE L0InterfaceStub::processClientSendRequest(CDataAccessor & accessor)
{
   ...
   if (SERVICE_CHANNEL_NUMBER !=channel_id)
   {
      LOG4CPLUS_INFO(logger, "L0InterfaceStub::processClientSendRequest() => Send Data Request: ChID = " + convertIntegerToString(channel_id));
      ret = sendData(accessor);
   }
   return ret;
}

ERROR_CODE L0InterfaceStub::processServiceAllocateRequest(CDataAccessor & accessor)
{
   ERROR_CODE ret = ERR_UNKNOWN;
   UInt32 prio = 0;
   memcpy(&prio, accessor.getData(), sizeof(UInt32));
   UInt32 channel_id = accessor.getChannelID();
   LOG4CPLUS_INFO(logger, "L0InterfaceStub::processServiceAllocateRequest()=>Allocate channel prio = "
         + convertIntegerToString(prio) + " id =" + convertIntegerToString(accessor.getChannelID()));

   bool found = false;
   AXIS::Ipc::DirectionID client_dir;
   ...
   mRequestedMapMutex.lock();
   {
      ...
      else
      {
         ...
         LOG4CPLUS_INFO(logger, "L0InterfaceStub::processServiceAllocateRequest()=>channel wasn't requested yet ->set channel to wait and trigger timeout");
         tRequestedChannelInfo info;
   ...
   if (found)
   {
      LOG4CPLUS_INFO(logger, "L0InterfaceStub::processServiceAllocateRequest()=>channel was already requested -> allocate channel");
      ...
      accessor.setData(reinterpret_cast<UInt8 *>(&data), sizeof(UInt32));
      sendRequest(accessor);
      LOG4CPLUS_INFO(logger, "L0InterfaceStub::processServiceAllocateRequest()=> after allocation send response to application!");
      UInt8* buf = new UInt8[accessor.getObjectSize()];
      accessor.copyToRawArray(buf);
...
ERROR_CODE L0InterfaceStub::processServiceDeallocateRequest(CDataAccessor & accessor)
{
   UInt32 channel_id = accessor.getChannelID();
   LOG4CPLUS_INFO(logger, "L0InterfaceStub::processServiceDeallocateRequest()=>Channel deallocate request id = " + convertIntegerToString(channel_id));
   ...
   if (ret == ERR_OK)
   {
      LOG4CPLUS_INFO(logger, "L0InterfaceStub::processServiceDeallocateRequest() => channel successfully deallocated. "
         "Notifying client");
      ...
      if (mRegistry.end() != iter)
      {
         ...
         LOG4CPLUS_TRACE(logger, "L0InterfaceStub::processServiceDeallocateRequest()=>channel found in registry - removing");
...
ERROR_CODE L0InterfaceStub::processServiceAllocateResponse(CDataAccessor & accessor)
{
   ERROR_CODE ret = ERR_UNKNOWN;
   UInt32 channel_id = accessor.getChannelID();
   UInt32 res = 0;
   memcpy(&res, accessor.getData(), sizeof(UInt32));
   LOG4CPLUS_INFO(logger, "L0InterfaceStub::processServiceAllocateResponse()=>Channel allocated responce id = "
   ...
   {
      LOG4CPLUS_INFO(logger, "L0InterfaceStub::processServiceAllocateResponse()=>Channel not found in requested map");
   }
   
   return ret;
}

ERROR_CODE L0InterfaceStub::processServiceDeallocateResponse(CDataAccessor & accessor)
{
   UInt32 channel_id = accessor.getChannelID();
   UInt32 res = 0;
   memcpy(&res, accessor.getData(), sizeof(UInt32));
   ...
   LOG4CPLUS_INFO(logger, "L0InterfaceStub::processServiceDeallocateResponse()=>Channel deallocated responce id = "
...
L0InterfaceStub::CRequestTimeout::CRequestTimeout(UInt32 timeout, tOpCode req, UInt32 channel_id, L0InterfaceStub* pOwner ,bool enabled):
   mRequest(req), mChannelID(channel_id), mpOwner(pOwner), mEnabled(enabled)
{
   CTimeoutManager::getInstance()->addSubscriber(this,timeout);
}

void L0InterfaceStub::CRequestTimeout::onTimeout()
{
   CDataAccessor resp;
   ...
   if (mEnabled)
   {
      switch (mRequest)
      {
         case E_ALLOCATE_CHANNEL:
         {
            ...
            if (found)
            {
               LOG4CPLUS_INFO(logger, "L0InterfaceStub::CRequestTimeout::onTimeout()=> channel "
                      + convertIntegerToString(mChannelID) + " allocated!");
            }
            else
            {
               LOG4CPLUS_WARN(logger, "L0InterfaceStub::CRequestTimeout::onTimeout()=> Channel "
         ...
         case E_DEALLOCATE_CHANNEL:
         {
            ...
            if (found)
            {
               LOG4CPLUS_INFO(logger, "L0InterfaceStub::CRequestTimeout::onTimeout()=> Channel deallocation timeout!");
               resp.setChannelID(mChannelID);
            ...
            {
               LOG4CPLUS_INFO(logger, "L0InterfaceStub::CRequestTimeout::onTimeout()=> channel Deallocated!");
            }
