...
Logger CTargetAgent::logger = Logger::getInstance(LOG4CPLUS_TEXT("ConnectivityAgent.L0.CTargetAgent"));
...
ERROR_CODE CTargetAgent::consumeBuffer(Buffer* pBuffer)
{
...
ERROR_CODE CTargetAgent::visitStub()
{
   ...
   if (buf)
   {
      ret = L0InterfaceStub::getInstance()->receiveData(*buf, mChannelID );
