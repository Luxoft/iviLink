...
public:
    ...
    * @param pTransmittedFrameQueue is the pointer to queue where the transmitted frames are stored in waiting for acknowledge
	 */
   CTransmitDispatcher(CCarrierAdapter* pCarrierAdaptor,CTransmittedFramesQueue* pTransmittedFrameQueue);
    ...
    * @return pointer to the created HAL source representation i.e. Source for L0 SourceAgent
	 */
   const CSource* openChannel(const UInt32 channel_id, const TChannelPriority prio, IBufferProducer& bufferProducer);
