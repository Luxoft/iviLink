...
* Company           LUXOFT
* @file             CQoS.cpp
* @brief            The CQoS class implementation file  
* @author           KSkidanov
***************************************************************************/
/********************************************************************
 *
  ...
  while (false == getStopFlag())
   {
      ...
      if (!mStatMap.empty())
      {
         ...
         while (iter != mStatMap.end())
         {
            ...
            {
               CChannelAllocator::getInstance()->narrowBandwidth(iter->first);
            }
            if (timeStat.mLowerTime > timeStat.getAverage())
            {
               CChannelAllocator::getInstance()->higherBandwidth(iter->first);
            }
            ++iter;
         }
      }
