 ...
 * (c) copyright     2011
 * Company           LUXOFT
 * @file             CTransmittedFramesQueue.cpp
 * @brief            The CTransmittedFramesQueue class implementation file
 * @author           KSkidanov
 ***************************************************************************/
/********************************************************************
 *
...
Logger CTransmittedFramesQueue::logger = Logger::getInstance(LOG4CPLUS_TEXT("ConnectivityAgent.HAL.CTransmittedFramesQueue"));

void CTransmittedFramesQueue::enqueue(Frame& frame)
{
...
Frame* CTransmittedFramesQueue::getFrameToRetransmit()
{
   mMutex.lock();
   if (!mQueue.empty())
   {
...
void CTransmittedFramesQueue::reenqueueRetransmittedFrame(Frame* pFrame)
{
   assert(!mQueue.empty());
   tFrameTimeStamped& fts = mQueue.front();
