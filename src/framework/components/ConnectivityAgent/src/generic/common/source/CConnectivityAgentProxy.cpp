...
* (c) copyright     2012
* Company           LUXOFT
* @file             CConnectivityAgentProxy.cpp
* @brief            The CConnectivityAgentProxy class implementation file
* @author           KSkidanov
***************************************************************************/
/********************************************************************
 *
 ...
 *
 ********************************************************************/
#include"framework/components/ConnectivityAgent/src/generic/common/include/CConnectivityAgentProxy.hpp"
/********************************************************************
 *
 * The other includes
 *
...
#include"framework/components/ConnectivityAgent/src/linux/config.hpp"
#include"framework/components/ConnectivityAgent/src/generic/common/include/CallbackWrappers.hpp"
...
void CConnectivityAgentProxy::OnConnection(AXIS::Ipc::DirectionID dirId)
{
   LOG4CPLUS_INFO(logger, "CConnectivityAgentProxy::OnConnection dirId = "
      + convertIntegerToString(dirId));
}

void CConnectivityAgentProxy::OnConnectionLost(AXIS::Ipc::DirectionID dirId)
{
...
void CConnectivityAgentProxy::OnRequest(AXIS::Ipc::MsgID id, 
   ...
   AXIS::Ipc::DirectionID dirId)
{
   ...
   UInt32 channel_id = responceDA.getChannelID();
   tOpCode type = static_cast<tOpCode>(responceDA.getOpCode());
   ...
   switch(type)
   {
      case E_ALLOCATE_CHANNEL_RESP:
      {
         ...
         mAllocateRequestResultCond.lock();
         CDataAccessor da(pPayload, payloadSize);
         mAllocateRequestResultMap[da.getChannelID()] = da;
         LOG4CPLUS_TRACE(logger, "broadcasting about channel " + convertIntegerToString(da.getChannelID()));
      ...
      case E_DEALLOCATE_CHANNEL_RESP:
      {
         ...
         mDeallocateRequestResultCond.lock();
         CDataAccessor da(pPayload, payloadSize);
         mDeallocateRequestResultMap[da.getChannelID()] = da;
         LOG4CPLUS_TRACE(logger, "broadcasting about channel " + convertIntegerToString(da.getChannelID()));
...
void CConnectivityAgentProxy::receiveDataNotification(CDataAccessor & accessor)
{
   ...
   if(iter != mRegistry.end())
   {
      ...
      else
      {
         ...
         free_size-=data_size;
         mCallbackListMutex.lock();
         CCallbackWrapper* pCallback = new CDataReceivedCallback(iter->second.mpClient,channel_id,data_size,free_size);
         mCallbackList.push_back(pCallback);
         mCallbackListMutex.unlock();
         mCallbackSema.signal();
      }
...
ERROR_CODE CConnectivityAgentProxy::sendData(UInt32 channel_id, UInt8* data, UInt32 size)
{
