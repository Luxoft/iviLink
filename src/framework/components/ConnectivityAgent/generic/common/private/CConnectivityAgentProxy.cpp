		...
		iviLink::Ipc::DirectionID dirId)
{
	LOG4CPLUS_TRACE_METHOD(logger, __PRETTY_FUNCTION__);
	LOG4CPLUS_INFO(logger,
			"CConnectivityAgentProxy::OnRequest(): size " + convertIntegerToString(payloadSize));
...
ERROR_CODE CConnectivityAgentProxy::receiveData(const UInt32 channel_id, UInt8* data,
...
ERROR_CODE CConnectivityAgentProxy::getConnectionInformation(ConnectionInformation* data)
{
	LOG4CPLUS_TRACE_METHOD(logger, __PRETTY_FUNCTION__);
	ERROR_CODE ret = ERR_FAIL;
