...
public:
	 ...
	 * @return ERR_OK if it's allright, ERR_NOTFOUND or ERR_FAIL if it's not
	 */
	ERROR_CODE sendData(UInt32 channel_id, UInt8 const* data, const UInt32 size);
	/**
	 ...
	 * @return ERR_OK if it's allright, ERR_NOTFOUND or ERR_FAIL if it's not
	 */
	ERROR_CODE receiveData(const UInt32 channel_id, UInt8* data, UInt32 & received_size,
	 ...
	 */
	ERROR_CODE getFreeSize(const UInt32 channel_id, UInt32& free_size) const;
