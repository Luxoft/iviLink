...
public:
    ...
    enum STATE
    {
        IDLE, NEGOTIATED, CHECKEDINMAP, CAALLOCATED, ALLOCDONE
    };
...
private:
    STATE GetTagStateInternal(const char * tag);
    Messages::Notification * GetNotificationInternal(const char * tag);
    CCondVar mStateCond;
    bool mWaitFlag;
    CRWMutex m_mapMutex;
    StateMap m_StateMap;
