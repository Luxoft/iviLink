...
pugi::xml_node Notification::AppendErrorNode(   pugi::xml_node node,
                                 ErrorCode errorCode,
...
void Notification::GetError(pugi::xml_node errorNode, ErrorCode & errorCode, const char *& errorString)
{
      ...
      m_tag(tag)
{
   ...
   {
      if (m_messageDocMainNode.append_attribute("name").set_value(m_notificationTypeMap[m_notificationType].c_str()))
      {
