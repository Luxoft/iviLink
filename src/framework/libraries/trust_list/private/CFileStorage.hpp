...
public:
	...
	virtual CTrustListError insert(CUid const& uid);
	virtual CTrustListError remove(CUid const& uid);
	virtual CTrustListError clear();
	virtual CTrustListError getOurUid(CUid& uid);
	virtual CTrustListError setOurUid(CUid const& uid);

public:

	CFileStorage(std::string const& filename);
	~CFileStorage();
