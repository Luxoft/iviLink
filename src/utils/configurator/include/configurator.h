...
namespace AXIS
{
	...
	public:
		...
		std::string filepath;
		std::list<AXIS::conf::kvpair> paramlist;		
	...
	public:
		...
		Configurator(std::string name)
		{
			...
			std::list<AXIS::conf::kvpair> pair = Parse(ReadFile("mconfig.ini")); 
			for (std::list<AXIS::conf::kvpair>::const_iterator i = pair.begin(); i != pair.end(); i++)
		    	{
		       		if (i->key == name)
				{
			    		filepath = i->value;
