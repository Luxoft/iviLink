...
namespace {
  Logger log = Logger::getInstance( LOG4CPLUS_TEXT("samples.Profiles.CClimateSenderProfile") );
}
              ...
              iviLink::Profile::IProfileCallbackProxy* pCbProxy)
{
  ...
  LOG4CPLUS_INFO(log, "ClimateSender: createProfile(\"" + id.value() + "\", \"" + sid.value() + "\")");

  return new iviLink::Profile::ProfileHolder(id, sid, new CClimateSenderProfile(pCbProxy));
}
